...
== Ausgangssituation
Der Content Manager Agent nutzt ein Claude Haiku 4.5 Model das auf dem SAP AI Core läuft. Dieses @LLM ist über eine mit @RAP veröffentlichte @ABAP\-Methode vom Frontend erreichbar.
Auf @img:cma_old_arch sind die bestehenden Tools des Content Manager Agents zu sehen. Weitere Tools können angeschlossen werden, indem diese als Client Tools in der Request definiert werden.
...
Für den Benutzer wird der Agent-Loop durch ein Schrittprotokoll visualisiert. Dieses zeigt in Echtzeit an, in welchem Schritt sich der Agent gerade befindet, etwa welches Tool aufgerufen wird oder ob auf eine Antwort gewartet wird. Dadurch bleibt der Verarbeitungsfortschritt transparent und erfüllt die Nielsen-Heuristik #1 (Sichtbarkeit des Systemstatus).
...
Der FeedbackHandler erstellt daraufhin ein @JSON\-Model mit den formularspezifischen Daten und einen dynamischen Controller mit den benötigten Event-Handlern. Die Dialog-Buttons werden mit Callback-Funktionen versehen, die bei Klick einen asynchronen Rückgabewert mit den eingegebenen Daten an den Content Manager Agent liefern. Das entsprechende @UI\5-Fragment wird geladen und in einem Dialog angezeigt. Der Content Manager Agent-Loop wartet währenddessen asynchron auf die Benutzerinteraktion.

Nachdem der Benutzer den Dialog abgeschlossen hat, sammelt der Controller die Formulardaten und gibt diese asynchron an den Content Manager Agent zurück, der damit seinen Loop fortsetzt. Wenn generierte Inhalte durch einen Klick auf den "Bestätigen"-Button freigegeben wurden, wird auf dem Backend bei dem Inhalt außerdem eine entsprechenden Flag auf "Accepted" gesetzt. Bei einem Klick auf den "Ablehnen"-Button wird diese Flag dahingegen auf "Rejected" gesetzt.

Die Aufteilung in die drei Komponenten _FeedbackTool_, _FeedbackHandler_ und _FeedbackDialogDefinitions_ bietet mehrere architektonische Vorteile. Durch die zentrale Verwaltung der Formulardefinitionen können neue Formulare hinzugefügt werden, ohne bestehende Logik anzupassen. Der _FeedbackHandler_ arbeitet generisch: Er erstellt Datenmodelle und Controller dynamisch basierend auf der übergebenen Konfiguration, wodurch formularspezifischer Code vermieden wird. Diese Trennung ermöglicht zudem eine unabhängige Testbarkeit der einzelnen Schichten. Die LLM-gestützte Formularauswahl ist dabei vollständig von der UI-Logik entkoppelt, was Änderungen an einer Komponente ohne Seiteneffekte auf andere erlaubt.

=== Formulare
Das Feedback-Tool unterstützt sieben verschiedene Formulare, die unterschiedliche Validierungs- und Feedback-Szenarien abdecken. Es wurden diese spezifischen Formulare entwickelt, um möglichst viele Situationen in den beiden Use-Cases Bildgenerierung und Textübersetzung abzudecken. Bilder der Formulare sind im Anhang unter @anhang:formulare zu finden.

*AgentQuestionForm* (@img:form_agent_question)*:* Das einfachste Formular stellt eine direkte Frage des Agents und bietet ein Textfeld für die Antwort. Es wird verwendet, wenn der Agent Klarstellung benötigt oder zusätzliche Informationen vom Benutzer einholen muss.

*RatingFeedbackForm* (@img:form_rating_feedback)*:* Dieses Formular zeigt ein einzelnes generiertes Bild zusammen mit einer 5-Sterne-Bewertung und einem Feedback-Textfeld. Der Benutzer kann das Bild akzeptieren, Anpassungen anfordern oder ablehnen. Es wird nach der Generierung einzelner Bilder verwendet, um detailliertes Feedback zur Qualität zu sammeln.
...
*ImageComparisonForm* (@img:form_image_comparison)*:* Bei diesem Formular werden zwei Bilder nebeneinander angezeigt. So können A/B-Tests durchgeführt werden, bei denen der Benutzer das bevorzugte Bild auswählen kann.

*MultiImageSelectionForm* (@img:form_multi_image_selection)*:* Dieses Formular präsentiert sechs generierte Bildvarianten. Der Benutzer kann eine oder mehrere passende Bilder auswählen oder alle ablehnen. Dieses Formular eignet sich für Szenarien, in denen aus mehreren Varianten die besten ausgewählt werden sollen.

*ImageStyleSelectionForm* (@img:form_image_style_selection)*:* Dieses Formular erlaubt es dem Nutzer vor der Inhaltsgenerierung Parameter wie Stil (fotorealistisch, illustrativ etc.), Perspektive, Farbschema, Hintergrund und Beleuchtung festzulegen. Dies ermöglicht eine präzisere Steuerung des Generierungsprozesses.

*ContentRefinementForm* (@img:form_content_refinement)*:* Dieses Formular zeigt ein Bild mit Schaltern und Eingabefeldern für verschiedene Aspekte (Farbe, Komposition, Beleuchtung, Details, Hintergrund). Der Benutzer kann spezifizieren, welche Aspekte beibehalten und welche geändert werden sollen. Dieses Formular unterstützt einen iterativen Verbesserungsprozess, bei dem das Bild schrittweise optimiert wird.


=== Integrationsmuster
Die Integration des Feedback-Tools folgt einem hybriden Ansatz, der Human-as-a-Tool und Interrupt-and-Resume kombiniert. Die Wahl des Integrationsmusters wird durch den System-Prompt des Content Manager Agents festgelegt und hängt vom Kontext ab. Dieser Prompt ist in @list:systemprompt im Anhang unter @anhang:systemprompt zu finden.
...
Diese Synergie ermöglicht es dem Agenten, autonom zu agieren, sofern möglich, und menschliches Urteilsvermögen einzuholen, falls notwendig.
